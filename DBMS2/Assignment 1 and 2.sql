...
FROM Members WHERE category = "F";

SELECT DISTINCT category FROM Members;

SELECT member_name, category FROM Members 
...
SELECT category, count(member_id) as CATEGORYCOUNT 
FROM Members
GROUP BY category;
...
FROM Members as m
INNER JOIN Members as e 
ON (m.category = e.category) WHERE e.member_name = "Gaurav Tak";


-- before performing select populating book_issue with data from book_return --
INSERT INTO Book_issue(issue_date, accession_no, member_id)
SELECT issue_date, accession_no, member_id FROM book_return;

-- needed query after each insert in book_issue --
UPDATE book_issue SET due_date = NOW() + INTERVAL 15 DAY where due_date is null; 

 -- actual select query - Query  for 9 --
...
ON bi.issue_date = br.issue_date 
